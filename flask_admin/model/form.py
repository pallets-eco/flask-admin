...
def create_editable_list_form(
    ...
    class ListForm(BaseListForm, form_base_class):  # type: ignore[misc, valid-type]
        """
        Also stores each field's original flask-admin widget (DatePickerWidget,
        etc.) before it gets replaced with the display widget (HTMXEditableWidget,
        by default) below. ajax_edit/ajax_update use `_original_widgets` to
        restore the real input widget once a cell is opened for editing.
        `_original_widgets` must stay per-invocation, so don't hoist this
        class definition out of the function unless you also make the dict
        instance-level instead of class-level.
        """
