...
class MockModelView(base.BaseModelView):
    ...
    def scaffold_list_columns(self):
        ...
        if self.column_exclude_list:
            return [x for x in columns if x not in self.column_exclude_list]
...
def test_column_filters():
    ...
    assert [(f['index'], f['operation']) for f in view._filter_groups['col1']], [(0 == 'test')]
    assert [(f['index'], f['operation']) for f in view._filter_groups['col2']], [(1 == 'test')]
...
def test_export_csv():
    ...
    data = rv.data.decode('utf-8')
    assert rv.status_code == 200
    strfield = "Str Field,Int Field\r\n" + "col1_1,2\r\n" + \
        "col1_2,4\r\n" + ",6\r\n"
    assert strfield == data
