...
def test_inline_form():
    ...
    rv = client.post('/admin/user/new/', data={'name': 'fbar', \
    ...
    assert rv.status_code == 200
    # Edit - update
    rv = client.post('/admin/user/edit/?id=2', data={'name': 'barfoo', \
                     'info-0-id': 1, 'info-0-key': 'xxx', 'info-0-val':'yyy'})
    assert UserInfo.query.count() == 1
    assert UserInfo.query.one().key == 'xxx'

    # Edit - add & delete
    rv = client.post('/admin/user/edit/?id=2', data={'name': 'barf', \
                     'del-info-0': 'on', 'info-0-id': '1', 'info-0-key': 'yyy', 'info-0-val': 'xxx',
                     'info-1-id': None, 'info-1-key': 'bar', 'info-1-val' : 'foo'})
    assert rv.status_code == 302
    assert User.query.count() == 2
    assert User.query.get(2).name == 'barf'
    assert UserInfo.query.count() == 1
