"""
Useful form fields for use with SQLModel ORM.
"""
...
class QuerySelectField(SelectFieldBase):
    ...
    SQLModel `Query`.  The `data` property actually will store/keep an ORM
        ...
        if self._object_list is None:
            ...
            for obj in query:
                if hasattr(obj, "__len__") and len(obj) > 0:
                    # It's a Row object, extract the first element
                    # which is the SQLModel instance
                    actual_obj = obj[0]
                    processed_objects.append((text_type(get_pk(obj)), actual_obj))
                else:
...
class InlineHstoreList(InlineFieldList):
    ...
    def process(self, formdata, data=unset_value, extra_filters=None):
        """SQLModel returns a dict for HSTORE columns, but WTForms cannot
        process a dict. This overrides `process` to convert the dict
        returned by SQLModel to a list of classes before processing."""
    ...
    if hasattr(obj, "__table__"):
        ...
        if isinstance(pk_names, tuple):
            ...
            for pk_name in pk_names:
                key_values.append(getattr(obj, pk_name))
